namespace LeetCode.T3001_T3500.T3301_T3400.T3341_FindMinimumTimeToReachLastRoomI;

public class T_FindMinimumTimeToReachLastRoomI
{
    public int MinTimeToReach(int[][] moveTime)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            var nextCells = new (int X, int Y)[4]
            {
                (cell.X, cell.Y - 1),
                (cell.X - 1, cell.Y),
                (cell.X + 1, cell.Y),
            ...
            foreach (var nextCell in nextCells)
            {
                ...
                var minTime = Math.Max(moveTime[nextCell.Y][nextCell.X] + 1, moveTime[cell.Y][cell.X] + 1);

                queue.Enqueue((nextCell.X, nextCell.Y), minTime);
